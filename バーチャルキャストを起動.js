 ...
 * @returns {boolean} 指定された値が Object であれば true を返します。
 */
function isValidConfig(config, filename)
{
	if (typeof config !== 'object' || config === null || Array.isArray(config)) {
		Shell.Popup(
			'「' + filename + '」のルートは object である必要がありますが、' + config + ' になっています。',
			0,
			WSH.ScriptName,
			vbOKOnly + vbCritical
		);
	...
	if (files.input) {
		...
		try {
			...
			if (exception.name === 'YAMLException') {
				...
				Shell.Popup(
					errorMessage + '¥n¥n' + exception.name + ': ' + exception.message,
					0,
					WSH.ScriptName,
					vbOKOnly + vbCritical
				);
		...
		putFileContents(
			folder.Path + '¥¥' + profileName + '_config.json', JSON.stringify(config, null, '¥t').replace(/¥n/g, '¥r¥n')
		);
	} else {
		...
		try {
			...
			Shell.Popup(
				'「' + files.output.Name + '」は壊れています。以下のエラーが発生しました。¥n¥n' + exception.name + ': ' + exception.message,
				0,
				WSH.ScriptName,
				vbOKOnly + vbCritical
			);
		...
		putFileContents(
			...
			jsyaml.safeDump(configJSON, {indent: 4, lineWidth: -1}).replace(/¥n/g, '¥r¥n')
		);
	}
}
...
/**
 * Executes es5-shim.js and es5-sham.js and other polyfills.
 * @returns {void}
 */
function polyfill()
{
...
(function (root, factory) {
    ...
    var isCallable; /* inlined from https://npmjs.com/is-callable */ var fnToStr = Function.prototype.toString, constructorRegex = /^¥s*class /, isES6ClassFn = function isES6ClassFn(value) { try { var fnStr = fnToStr.call(value); var singleStripped = fnStr.replace(/¥/¥/.*¥n/g, ''); var multiStripped = singleStripped.replace(/¥/¥*[.¥s¥S]*¥*¥//g, ''); var spaceStripped = multiStripped.replace(/¥n/mg, ' ').replace(/ {2}/g, ' '); return constructorRegex.test(spaceStripped); } catch (e) { return false; /* not a function */ } }, tryFunctionObject = function tryFunctionObject(value) { try { if (isES6ClassFn(value)) { return false; } fnToStr.call(value); return true; } catch (e) { return false; } }, fnClass = '[object Function]', genClass = '[object GeneratorFunction]', isCallable = function isCallable(value) { if (!value) { return false; } if (typeof value !== 'function' && typeof value !== 'object') { return false; } if (hasToStringTag) { return tryFunctionObject(value); } if (isES6ClassFn(value)) { return false; } var strClass = to_string.call(value); return strClass === fnClass || strClass === genClass; };
...
/**
 * @see [Array.prototype.includes() - JavaScript | MDN]{@link
...
ESCAPE_SEQUENCES[0x09]   = '¥¥t';
ESCAPE_SEQUENCES[0x0A]   = '¥¥n';
...
// Indents every line in a string. Empty lines (¥n only) are not indented.
function indentString(string, spaces) {
  ...
  while (position < length) {
    next = string.indexOf('¥n', position);
    if (next === -1) {
    ...
    if (line.length && line !== '¥n') result += ind;
...
function generateNextLine(state, level) {
  return '¥n' + common.repeat(' ', state.indent * level);
}
...
// Derived from nb-char - ¥t - #x85 - #xA0 - #x2028 - #x2029.
...
// Post-conditions:
//    STYLE_PLAIN or STYLE_SINGLE => no ¥n are in the string.
...
function chooseScalarStyle(string, singleLineOnly, indentPerLevel, lineWidth, testAmbiguousType) {
  ...
  if (singleLineOnly) {
    ...
    // in case the end is missing a ¥n
    hasFoldableLine = hasFoldableLine || (shouldTrackWidth &&
      (i - previousLineBreak - 1 > lineWidth &&
       string[previousLineBreak + 1] !== ' '));
  }
  // Although every style can represent ¥n without escaping, prefer block styles
...
function blockHeader(string, indentPerLevel) {
  ...
  // note the special case: the string '¥n' counts as a "trailing" empty line.
  var clip =          string[string.length - 1] === '¥n';
  var keep = clip && (string[string.length - 2] === '¥n' || string === '¥n');
  var chomp = keep ? '+' : (clip ? '' : '-');

  return indentIndicator + chomp + '¥n';
}

// (See the note for writeScalar.)
function dropEndingNewline(string) {
  return string[string.length - 1] === '¥n' ? string.slice(0, -1) : string;
}
...
function foldString(string, width) {
  ...
  var result = (function () {
    var nextLF = string.indexOf('¥n');
    nextLF = nextLF !== -1 ? nextLF : string.length;
    lineRe.lastIndex = nextLF;
    return foldLine(string.slice(0, nextLF), width);
  }());
  // If we haven't reached the first content line yet, don't add an extra ¥n.
  var prevMoreIndented = string[0] === '¥n' || string[0] === ' ';
  var moreIndented;
...
// NB. More-indented lines *cannot* be folded, as that would add an extra ¥n.
function foldLine(line, width) {
  if (line === '' || line[0] === ' ') return line;

  // Since a more-indented line adds a ¥n, breaks can't be followed by a space.
  ...
  while ((match = breakRe.exec(line))) {
    ...
    if (next - start > width) {
      ...
      result += '¥n' + line.slice(start, end);
      // skip the space that was output as ¥n
  ...
  if (line.length - start > width && curr > start) {
    result += line.slice(start, curr) + '¥n' + line.slice(curr + 1);
  } else {
    result += line.slice(start);
  }

  return result.slice(1); // drop extra ¥n joiner
}
...
function dump(input, options) {
  ...
  if (writeNode(state, 0, input, true, true)) return state.dump + '¥n';
...
var PATTERN_NON_PRINTABLE         = /[¥x00-¥x08¥x0B¥x0C¥x0E-¥x1F¥x7F-¥x84¥x86-¥x9F¥uFFFE¥uFFFF]|[¥uD800-¥uDBFF](?![¥uDC00-¥uDFFF])|(?:[^¥uD800-¥uDBFF]|^)[¥uDC00-¥uDFFF]/;
var PATTERN_NON_ASCII_LINE_BREAKS = /[¥x85¥u2028¥u2029]/;
...
function writeFoldedLines(state, count) {
  if (count === 1) {
    ...
    state.result += common.repeat('¥n', count - 1);
  }
}
  ...
  while (ch !== 0) {
    ...
    if (state.lineIndent < textIndent) {
      ...
      if (chomping === CHOMPING_KEEP) {
        state.result += common.repeat('¥n', didReadContent ? 1 + emptyLines : emptyLines);
    ...
    if (folding) {
      ...
      // End of more-indented block.
        ...
        atMoreIndented = false;
        state.result += common.repeat('¥n', emptyLines + 1);
      ...
      // Several line breaks - perceive as different lines.
        ...
        state.result += common.repeat('¥n', emptyLines);
      }
...
Mark.prototype.getSnippet = function getSnippet(indent, maxLength) {
  ...
  while (start > 0 && '¥x00¥r¥n¥x85¥u2028¥u2029'.indexOf(this.buffer.charAt(start - 1)) === -1) {
    start -= 1;
  ...
  while (end < this.buffer.length && '¥x00¥r¥n¥x85¥u2028¥u2029'.indexOf(this.buffer.charAt(end)) === -1) {
    end += 1;
  ...
  return common.repeat(' ', indent) + head + snippet + tail + '¥n' +
...
var BASE64_MAP = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=¥n¥r';
...
function constructYamlBinary(data) {
  var idx, tailbits,
      input = data.replace(/[¥r¥n=]/g, ''), // remove CR/LF & padding to simplify scan
      max = input.length,
      map = BASE64_MAP,
      bits = 0,
      result = [];
...
var YAML_TIMESTAMP_REGEXP = new RegExp(
  ...
  ':([0-9][0-9])'                    + // [6] second
  '(?:¥¥.([0-9]*))?'                 + // [7] fraction
  '(?:[ ¥¥t]*(Z|([-+])([0-9][0-9]?)' + // [8] tz [9] tz_sign [10] tz_hour
